...
constraint. The complete model uses \textbf{24\,181 parameters}, between one half
and one eighteenth of the convolutional and transformer baselines we compare
against. On a 7-subject, 9-session exoskeleton cohort it reaches 0.901 balanced
accuracy with expected calibration error 0.039 and 0.876 accuracy at 90\,\%
coverage, while removing 52\,\% of session-to-session covariance drift; on an
independent 8-subject treadmill cohort the drift reduction is 84\,\%, significant
in 15 of 15 subjects across both cohorts ($p \le 10^{-4}$) against a no-op
control. We do not claim state-of-the-art accuracy: a strong attention baseline
scores higher in the same harness, and we report that directly. What we do
establish is a design condition for in-network adaptation. The running estimate
tracks whatever is currently streaming, so the adaptation rate must be fast
enough to follow session drift yet slow enough not to follow the class itself.
We show the two cohorts' optimal rates are opposite, each losing 0.06--0.09 at
the other's setting, and that where a protocol's class blocks outlast its drift
timescale no valid rate exists. Both timescales are computable from the recording
protocol before a model is fitted.
\end{abstract}
