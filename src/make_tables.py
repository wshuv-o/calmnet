...
# No table in the manuscript now reports an arm built from a component
# outside the proposed model, so the marker that named them is gone.
...
def table(label, caption, colspec, header, rows, wide=False, small=True,
    ...
    if fill:
        ...
        if fit:
            out.append(BS + "resizebox{" + BS + ("textwidth" if wide else "columnwidth") + "}{!}{%")
...
def t_ablation3():
    ...
    for lab, (arm, files) in ABL3_SRC.items():
        ...
        for f in files:
            for k, v in L(f).items():
                if k.split("|")[0] == arm and k not in runs:
                    runs[k] = v
...
def main():
    parts = ["%% Generated by src/make_tables.py -- do not edit by hand." + NL + NL,
             # t_ablation3, t_rate, t_ratecurve and t_drift compare configurations
             # built from components that are not in the reported model. They are
             # no longer emitted; the functions stay in this file so nothing is
