...
For a patient who cannot walk after a spinal cord injury or a
...
\caption{The CALM-Net framework. Multimodal encoders map EEG sub-bands, IMU, and
...
\caption{Motion-invariant disentanglement across seven subjects. \textbf{Left:}
...
Table~\ref{tab:full} reports these per subject, averaged over the six held-out test
sessions with training on sessions 1--3. There, invAcc is movement-invariant balanced
accuracy; ECE$_{\text{raw}}$ and ECE$_{\text{cal}}$ are expected calibration error
before and after temperature scaling; exec@80 is balanced accuracy over the $80\%$
most-confident committed windows; and cov$_{\text{stat}}$ and cov$_{\text{adpt}}$ are
static and adaptive conformal coverage against a $0.90$ target.

\begin{table*}[t]
\caption{Full CALM-Net per subject, mean over the six held-out test sessions with
...
\toprule
Method & Inv. & Acc & ECE & AUROC & Cov. \\
\midrule
IMU-only & -- & 0.839 & -- & -- & -- \\
EEGNet~\cite{lawhern2018} & no & 0.819 & 0.086 & 0.813 & no \\
EEG Conf.~\cite{song2023} & no & 0.826 & 0.067 & 0.845 & no \\
\textbf{CALM-Net} & \textbf{yes} & \textbf{0.694} & 0.114 & 0.698 & \textbf{yes} \\
\bottomrule
\end{tabular}
\end{table}
...
flatters the original analysis (Table~\ref{tab:probe}).
...
Accuracy and leakage correlate at $r = +0.68$ (Table~\ref{tab:topcfg}).
...
The aggregate $-0.265$ conceals a split population (Table~\ref{tab:persubj}).
...
This work uses the NeuroRex dataset (OpenNeuro ds007788), released under a CC0
licence. Code and the complete result files underlying every number reported
here, including the per-seed architecture sweep and the invariance-probe
implementation, are available in the project repository.
